/*!=============================================================================
  ...
  Example demonstrating minimum jerk trajectory generation.
 ...
 * 
 * \param[in,out] state   Current state (position, velocity, acceleration)
 * \param[in]     goal    Goal state
 * \param[in]     tau     Time remaining to reach goal
 * \param[in]     dt      Time step
 * \return                TRUE if successful, FALSE if tau < dt
 */
int calculate_min_jerk_next_step(double *pos, double *vel, double *acc,
                                  double goal_pos, double goal_vel, double goal_acc,
    ...
    double dist = goal_pos - *pos;
    double a1t2 = goal_acc;
    double a0t2 = *acc;
    double v1t1 = goal_vel;
    double v0t1 = *vel;
    ...
    *pos = c1 * t5 + c2 * t4 + c3 * t3 + c4 * t2 + c5 * t1 + c6;
    *vel = 5.0 * c1 * t4 + 4.0 * c2 * t3 + 3.0 * c3 * t2 + 2.0 * c4 * t1 + c5;
    *acc = 20.0 * c1 * t3 + 12.0 * c2 * t2 + 6.0 * c3 * t1 + 2.0 * c4;
...
int main(int argc, char **argv) {
    ...
    /* ----------------------------------------------------------------
     * Example 2: Joint Trajectory using SL-style Algorithm
     * ----------------------------------------------------------------
     * Use the iterative approach similar to SL's goto_task.
     */
    
    printf("\n--- Example 2: Iterative Min-Jerk (SL-style) ---\n");
    
    double pos = 0.0;         /* Current position (radians) */
    double vel = 0.0;         /* Current velocity (rad/s) */
    double acc = 0.0;         /* Current acceleration (rad/s^2) */
    double goal_pos = PI / 2; /* Target: 90 degrees */
    double goal_vel = 0.0;
    double goal_acc = 0.0;
    double total_time = 1.0;  /* 1 second movement */
    double time_step = 0.01;  /* 100 Hz servo rate */
    
    printf("\nJoint trajectory: 0 -> 90 degrees in 1 second\n");
    printf("  Servo rate: 100 Hz\n");
    ...
    while (time_to_go > time_step) {
        if (step % print_interval == 0) {
            ...
            printf("  %5.2f s   %10.4f      %10.4f\n", 
                   elapsed, pos * 180.0 / PI, vel * 180.0 / PI);
        }
        
        calculate_min_jerk_next_step(&pos, &vel, &acc,
    ...
    /* Print final state */
    printf("  %5.2f s   %10.4f      %10.4f\n", 
           total_time, pos * 180.0 / PI, vel * 180.0 / PI);
    ...
    /* ----------------------------------------------------------------
     * Example 3: Multi-DOF Trajectory
     * ----------------------------------------------------------------
     * Generate coordinated trajectories for multiple joints.
     */
    ...
    double goals[3] = {PI / 4, PI / 3, PI / 6};  /* 45, 60, 30 degrees */
    char *joint_names[3] = {"Shoulder", "Elbow", "Wrist"};
    ...
    while (time_to_go > time_step) {
        int j;
        for (j = 0; j < n_dof; j++) {
            calculate_min_jerk_next_step(&positions[j], &velocities[j], &accelerations[j],
                                         goals[j], 0.0, 0.0,
                                         time_to_go, time_step);
        }
        time_to_go -= time_step;
    ...
    int j;
    for (j = 0; j < n_dof; j++) {
        double target_deg = goals[j] * 180.0 / PI;
        double achieved_deg = positions[j] * 180.0 / PI;
        double error_deg = target_deg - achieved_deg;
        printf("  %-10s %10.4f   %10.4f     %10.6f\n",
               joint_names[j], target_deg, achieved_deg, error_deg);
    }
    ...
    pos = 0.5;          /* Starting at 0.5 rad */
    vel = 0.2;          /* Already moving at 0.2 rad/s */
    acc = 0.0;
    goal_pos = 1.5;     /* Target position */
    total_time = 1.0;
    ...
    compute_min_jerk_coefficients(pos, vel, acc, goal_pos, 0.0, 0.0, total_time, c);
